function readopmF() {
    ...
    if(document.querySelector("div.card-table") !== null){
        allChaptersPage = true;
    }
    // check if viewing chapters list page
    if(document.querySelector("div.card-table > div.card-body > table.table") !== null){
        rows = document.querySelectorAll("div.card-table > div.card-body > table.table > tbody > tr");
        if(allChaptersPage){
            addNote();
            addBatchDownload();
        }
    ...
    function addDownloadButtons(){
        for (let i=0;i<rows.length;i++) {
            ...
            // get title
            let title = rows[i].firstElementChild.textContent;
            pdfButton.title = title;
            zipButton.title = title;
            // store chapter link
            links[i] = rows[i].querySelector("a.btn-primary").href;
            pdfButton.referrerLink = rows[i].querySelector("a.btn-primary").href;
            let holder = rows[i].querySelector("a.btn-primary").parentElement.parentElement;
