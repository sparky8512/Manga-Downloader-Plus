function kiryuuF() {
    ...
    if(document.querySelector("div#chapterlist") !== null){
        rows = document.querySelectorAll("div#chapterlist > ul > li");
        // calling function
        addNote();
        addBatchDownload();
        addDownloadButtons();
    ...
    function addNote(){
        let chapterDiv = document.querySelector("div.bixbox.bxcl");
    ...
    function addDownloadButtons(){
        for (let i=0;i<rows.length;i++) {
            ...
            // store chapter link
            links[i] = rows[i].querySelector("div.eph-num a").href;
            pdfButton.referrerLink = rows[i].querySelector("div.eph-num a").href;

            pdfButton.title = rows[i].querySelector("div.eph-num span.chapternum").textContent.trim();
            zipButton.title = pdfButton.title;
            
            let buttonHolder = rows[i].querySelector("div.dt");
