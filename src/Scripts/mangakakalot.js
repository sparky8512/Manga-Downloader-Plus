function mangakakalotF() {
    ...
    if(document.querySelector("div.manga-info-chapter") !== null){
        rows = document.querySelectorAll("div.chapter-list > div.row");
        addNote();
        addBatchDownload();
        addDownloadButtons();
    }
    ...
    function addDownloadButtons(){
        for (let i=0;i<rows.length;i++) {
            ...
            rows[i].classList.add("md-mangakakalot-highlight");
            // store chapter link
    ...
    async function getChaptersData(){
        ...
        let server = "https://mangakakalot.com/change_content_s1";
        for(let i=0;i<rows.length;i++){
            // store chapter link
